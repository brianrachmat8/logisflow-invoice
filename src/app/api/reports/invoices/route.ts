...
export async function GET() {
  ...
  try {
    ...
    sheet.columns = [
      ...
      { header: "Klien", key: "client", width: 30 }, { header: "Job Order", key: "job", width: 24 },
    ...
    invoices.forEach((invoice) => sheet.addRow({
      number: invoice.invoiceNumber || invoice.draftNumber, date: invoice.invoiceDate, client: invoice.client.name,
      job: invoice.shipment.jobNumber, bill: invoice.bill?.number || "-", type: invoice.type,
      subtotal: invoice.subtotal.toNumber(), tax: invoice.taxAmount.toNumber(), total: invoice.grandTotal.toNumber(),
      outstanding: invoice.outstandingAmount.toNumber(), status: invoice.status,
    }));

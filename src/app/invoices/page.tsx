  ...
  return <AppShell title="Invoice">
    ...
    <div className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Nomor invoice</th><th>Klien</th><th>Job / Referensi</th><th>Tanggal</th><th>Tipe</th><th>Total</th><th>Status</th></tr></thead><tbody>
        {invoices.map((item) => {
          const isManual = item.type === "LAIN_LAIN";
          return <tr key={item.id}>
            ...
            <td>{item.client.name}</td><td>{isManual ? (item.manualReference || item.manualTitle || "Invoice Lain-lain") : item.shipment?.jobNumber}<br/><small style={{ color: "var(--muted)" }}>{isManual ? "Non-trucking" : item.bill?.number || "Gabungan"}</small></td>
            <td>{tanggal.format(item.invoiceDate)}</td><td>{isManual ? "LAIN-LAIN" : item.type}</td><td className="money">{rupiah.format(numberValue(item.grandTotal))}</td><td><StatusBadge status={item.status} /></td>
          </tr>;
        })}

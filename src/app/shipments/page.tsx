...
export default async function ShipmentsPage() {
  ...
  return <AppShell title="Shipment / Job Order">
    ...
    <div className="card">
      <div className="filters"><input className="filter-input search" placeholder="Cari job, DO, atau klien..." /><select className="filter-input"><option>Semua status</option></select></div>
      <div className="table-wrap"><table><thead><tr><th>Job order</th><th>Klien</th><th>Vessel / Voyage</th><th>B/L</th><th>Kontainer</th><th>Tanggal</th><th>Status</th><th>Aksi</th></tr></thead>
        <tbody>{shipments.map((item) => {
          ...
          return <tr key={item.id}>
            <td className="primary-cell"><Link href={`/shipments/${item.id}`}><strong>{item.jobNumber}</strong><span>{item.shipmentDirection === "EXPORT" ? "Export DO" : "Import B/L"} {item.doNumber}</span></Link></td>
            <td>{item.client.name}</td><td>{item.vessel} / {item.voyage}<br /><small style={{ color: "var(--muted)" }}>{item.carrier?.name || "-"}</small></td>
            <td>{item._count.bills}</td><td>{item._count.containers}</td><td>{tanggal.format(item.shipmentDate)}</td><td><StatusBadge status={item.status} /></td>
